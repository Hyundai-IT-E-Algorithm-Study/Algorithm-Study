...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		for(int i=1; i<N; i++) {
			...
			//이 값보다 강의 시작시간이 빠르다면, 
			//이것은 현재 우선순위큐에 있는 모든 방에 다 못들어 간다는 의미(시작점은 정렬상 뒤인데 끝나는점은 빠르기때문)
			//따라서 새로운 방을 만들어줘서 끝나는 시간 넣어줌
			if (room.peek() > lecture[i][0]) {
				room.offer(lecture[i][1]);
			}
			else {
			//시작시간이 뒤라면 현재 강의실에 들어갈 수 있음
     (다른 강의실에 들어갈수있는지 여부는 알 수 없음
      ->따라서 현재 강의실에 들어가야 최선의 선택!)
			//따라서 가장 종료시간 빠른 강의를 큐에서 빼내고
			//해당 끝나는 시간을 넣어줌으로서 강의실의 현시점 강의 교체
				room.poll();
				room.offer(lecture[i][1]);
			}
		}
